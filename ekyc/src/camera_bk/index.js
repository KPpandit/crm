...
export function Camera({ onCapture, onClear, changekaru }) {
    ...
    function handleCapture() {
        ...
        if (idcnt === true) {
            ...
            if(From == "Addressproof"){
                var dataURL2 = canvasRef.current.toDataURL('image/jpeg');
                var imgId7 = dataURL2.replace(/^data:image\/(png|jpeg);base64,/, '');
                localStorage.setItem('imgId7', imgId7);
            }
            if(From == "backcamera"){
                var dataURL3 = canvasRef.current.toDataURL('image/jpeg');
                var imgId9 = dataURL3.replace(/^data:image\/(png|jpeg);base64,/, '');
                localStorage.setItem('imgData', imgId9);
            }
            if(From == "Permit"){
                var dataURL4 = canvasRef.current.toDataURL('image/jpeg');
                var imgId8 = dataURL4.replace(/^data:image\/(png|jpeg);base64,/, '');
                localStorage.setItem('imgId8', imgId8);
            }
            
            if(From == 'consentForm'){
                var dataURL5 = canvasRef.current.toDataURL('img/jpeg');
                var imgId10 = dataURL5.replace(/^data:image\/(png|jpeg);base64,/, '');
                localStorage.setItem('imgId10', imgId10);
            }
