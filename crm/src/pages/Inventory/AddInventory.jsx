...
export default function AddInventory() {
    ...
    const { handleChange, handleSubmit, handleBlur, values } = useFormik({
        ...
        onSubmit: async (values) => {
             console.log(values.vendor_id);
            // setResult(values);
            const res = await axios.post('http://172.5.10.2:9696/api/inventory/detail/save',
                { ...values }, {
                headers: {
                ...
                if (res.status === 200) {
                    toast.success('Inventory Record Added Successfully', { autoClose: 2000 });
                }
                // location.reload();
            }).catch(err => {
                // console.log("-----------");
