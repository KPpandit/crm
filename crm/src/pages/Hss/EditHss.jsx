...
export default function Edithss() {
    ...
    const { handleChange, handleSubmit, handleBlur, values, setValues } = useFormik({
        ...
        onSubmit: async (values) => {
            // console.log(values);
            // setResult(values);
            const res = await axios.put('http://172.5.10.2:9696/api/hss/detail/update?imsi=' + selectObj.imsi + '&msisdn=' + selectObj.msisdn,
                { ...values }, {
                headers: {
                ...
                if (res.status === 200) {
                    toast.success('UDM Record Updated Successfully', { autoClose: 2000 });
                }
                // location.reload();
            }).catch(err => {
                console.log("-----------");
    ...
    useEffect(() => {
        if (selectObj) {
            setValues((prevValues) => ({
                ...
                ambr: selectObj.ambr || '',
                nssai: selectObj.nssai || '',
                arfb: selectObj.arfb || '',
                sar: selectObj.sar || '',
                rat: selectObj.rat || '',
                cn: selectObj.cn || '',
                smf_sel: selectObj.smf_sel || '',
                sm_dat: selectObj.sm_dat || '',
                eps_flag: selectObj.eps_flag || '',
                eps_odb: selectObj.eps_odb || '',
                hplmn_odb: selectObj.hplmn_odb || '',
                ard: selectObj.ard || '',
                epstpl: selectObj.epstpl || '',
                status: selectObj.status || '',
                context_id: selectObj.context_id || '',
                apn_context: selectObj.apn_context || '',
            }));
        }
    }, [selectObj]);
    console.log(values)
